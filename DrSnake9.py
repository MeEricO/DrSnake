...
from LeftGreenBlock import LeftGreenBlock
from RightGreenBlock import RightGreenBlock
...
leftGreenList = pygame.sprite.Group()
rightGreenList = pygame.sprite.Group()
...
while not done:
    ...
    for block in leftRedHits:
        ...
        if totalLeftReds % 20 == 0:
            if totalLeftReds > 20:
                allSpritesList.remove(leftGreen)
            leftGreenList = pygame.sprite.Group()
            leftGreenx = random.randint(20, 760)
            while leftGreenx % 20 != 0:
                leftGreenx = random.randint(20, 760)
            leftGreeny = random.randint(20, 760)
            while leftGreeny % 20 != 0:
                leftGreeny = random.randint(20, 760)
            leftGreen = LeftGreenBlock(leftGreenx, leftGreeny)
            leftGreenList.add(leftGreen)
            allSpritesList.add(leftGreen)
    leftRedHits = False
    ...
    for block in rightRedHits:
        ...
        if totalRightReds % 15 == 0:
            if totalRightReds > 15:
        ...
        if totalRightReds % 20 == 0:
            if totalRightReds > 20:
                allSpritesList.remove(rightGreen)
            rightGreenList = pygame.sprite.Group()
            rightGreenx = random.randint(820, 1560)
            while rightGreenx % 20 != 0:
                rightGreenx = random.randint(820, 1560)  
            rightGreeny = random.randint(20, 760)
            while rightGreeny % 20 != 0:
                rightGreeny = random.randint(20, 760)
            rightGreen = RightGreenBlock(rightGreenx, rightGreeny)
            rightGreenList.add(rightGreen)
            allSpritesList.add(rightGreen)
    ...
    # ------------------Adding length power up(Green blocks)---------------
    # Left --------------------------------
    leftGreenHits = pygame.sprite.spritecollide(leftSnake, leftGreenList, True)
    for block in leftGreenHits:
        oldLength = len(leftSnakeList)
        allSpritesList.remove(leftSnakeList)
        leftSnakeList = []
        leftSnakeList.append(leftSnake)
        for seg in range(oldLength + 50):
            x = leftSnakeList[0].rect.x 
            y = leftSnakeList[0].rect.y 
            leftAdd = LeftSnake(x, y)
            leftSnakeList.append(leftAdd)
            allSpritesList.add(leftAdd)
        leftGreenList.remove(leftGreen)

    # Right --------------------------------
    rightGreenHits = pygame.sprite.spritecollide(rightSnake,
                                                 rightGreenList, True)
    for block in rightGreenHits:
        oldLength = len(rightSnakeList)
        allSpritesList.remove(rightSnakeList)
        rightSnakeList = []
        rightSnakeList.append(rightSnake)
        for seg in range(oldLength + 50):
            ...
            rightSnakeList.append(rightAdd)
            allSpritesList.add(rightAdd)
        rightGreenList.remove(rightGreen)
    # ---------------------------------------------------------------------
